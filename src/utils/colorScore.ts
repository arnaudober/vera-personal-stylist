...
export type PreparedColor = {
    L: number; a: number; b: number;      // Lab (float)
    h: number; s: number; v: number;      // HSV (0..360, 0..1, 0..1)
};
...
export function findClosestColorPair(items: ColorSet): { item1: Item; item2: Item; distance: number } | null {
    ...
    for (let i = 0; i < items.length; i++) {
        for (let j = i + 1; j < items.length; j++) {
            ...
            if (distance < minDistance) {
                ...
                closestPair = {
                    item1: items[i], item2: items[j], distance
                };
            }
        }
    }
...
function colorDistance(color1: PreparedColor, color2: PreparedColor): number {
    return deltaE2000_Lab(color1.L, color1.a, color1.b, color2.L, color2.a, color2.b);
}
...
function deltaE2000_Lab(L1: number, a1: number, b1: number, L2: number, a2: number, b2: number): number {
...
function rgbToLab(r255: number, g255: number, b255: number) {
    ...
    const L = 116 * fy - 16, a = 500 * (fx - fy), bl = 200 * (fy - fz);
    return {L, a, b: bl};
}